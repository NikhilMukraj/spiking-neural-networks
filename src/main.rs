...
// fn decay_scale(input_value: f64, pre_exp_scale: f64, scalar: f64) {
//     scalar * (-1. * input_value.abs() / pre_exp_scale).exp()
// }

// fn add_spike_time(&mut self, spike_time: usize, window: usize) {
//     for i in self.spike_times {
//         if spike_time - window < i {
//             self.spike_times.remove(0);
//         }
//     }

//     self.spike_times.add(spike_time)
// }

// fn spikes_in_window(&self, now: usize, window: usize) -> usize {
//     self.spike_times.filter(|i| now - window >= i)
//         .collect::<Vec<usize>>()
//         .len()
// }

// fn neuromodulate_bursting(
//     presynaptic_neuron: &mut Cell, 
//     postsynaptic_neuron: &mut Cell, 
//     neuromodulatory_params: &NeuromodulatoryParams,
// ) {
//     // tonic
//     // dt = 0.5
//     // a = 0.01
...
//     // or determine number of spikes difference
//     // spikes window, vec of times
//     // only include times that are greater than or equal to now - spikes window
//     // if less than that window, remove from vector
//     // if pre - post is negative, move toward tonic
//     // if pre - post is positive, move toward bursting

//     let pre_spikes = presynaptic_neuron.spikes_in_window(neuromodulatory_params.window);
//     let post_spikes = postsynaptic_neuron.spikes_in_window(neuromodulatory_params.window);

//     let diff = (pre_spikes - post_spikes) as f64;

//     if diff < 0 {
//         postsynaptic_neuron.alpha = max(
//             neuromodulatory_params.tonic_alpha, 
//             postsynaptic_neuron.alpha - decay_scale(diff, neuromodulatory_params.scale_a_plus, 1.0)
//         );
//         postsynaptic_neuron.c = max(
//             neuromodulatory_params.tonic_c, 
//             postsynaptic_neuron.c - decay_scale(diff, neuromodulatory_params.scale_c_plus, 1.0)
//         );
//         postsynaptic_neuron.d = max(
//             neuromodulatory_params.tonic_d, 
//             postsynaptic_neuron.d - decay_scale(diff, neuromodulatory_params.scale_d_plus, 1.0)
//         );
//     } else if diff > 0 {
//         postsynaptic_neuron.alpha = max(
//             neuromodulatory_params.tonic_alpha, 
//             postsynaptic_neuron.alpha + decay_scale(diff, neuromodulatory_params.scale_a_minus, -1.0)
//         );
//         postsynaptic_neuron.c = max(
//             neuromodulatory_params.tonic_c, 
//             postsynaptic_neuron.c + decay_scale(diff, neuromodulatory_params.scale_c_minus, -1.0)
//         );
//         postsynaptic_neuron.d = max(
//             neuromodulatory_params.tonic_d, 
//             postsynaptic_neuron.d + decay_scale(diff, neuromodulatory_params.scale_d_minus, -1.0)
//         );
//     } 
// }
