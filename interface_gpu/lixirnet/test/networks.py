...
class TestCPUGPUImpl(unittest.TestCase):
    def test_network_isolated_electrical_using_from(self):
        ...
        # init_state1 = np.random.uniform(1, -1, (exc_n1, exc_n1))
        # init_state2 = np.random.uniform(1, -1, (exc_n2, exc_n2))
        # init_state1 = np.full((exc_n1, exc_n1), -50)
        # init_state2 = np.full((exc_n1, exc_n1), -20)
        ...
        for n1, n2 in zip(range(exc_n1), range(exc_n1)):
            for m1, m2 in zip(range(exc_n1), range(exc_n1)):
                self.assertTrue(
                    np.abs(network.get_lattice(e1).get_weight((n1, m1), (n2, m2)) - gpu_network.get_lattice(e1).get_weight((n1, m1), (n2, m2))) < 1e-5
                )

        for n1, n2 in zip(range(exc_n1), range(exc_n1)):
            for m1, m2 in zip(range(exc_n1), range(exc_n1)):
                self.assertTrue(
                    np.abs(network.get_lattice(e1).get_neuron(n1, m1).current_voltage - gpu_network.get_lattice(e1).get_neuron(n2, m2).current_voltage) < 1e-5,
        ...
        for n1, n2 in zip(range(exc_n2), range(exc_n2)):
            for m1, m2 in zip(range(exc_n2), range(exc_n2)):
                self.assertTrue(
                    np.abs(network.get_lattice(e2).get_weight((n1, m1), (n2, m2)) - gpu_network.get_lattice(e2).get_weight((n1, m1), (n2, m2))) < 1e-5
                )

        for n1, n2 in zip(range(exc_n2), range(exc_n2)):
            for m1, m2 in zip(range(exc_n2), range(exc_n2)):
                self.assertTrue(
                    np.abs(network.get_lattice(e2).get_neuron(n1, m1).current_voltage - gpu_network.get_lattice(e2).get_neuron(n2, m2).current_voltage) < 1e-5,
