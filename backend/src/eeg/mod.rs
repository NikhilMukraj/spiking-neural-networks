...
use crate::error::TimeSeriesProcessingErrorKind;
...
pub fn power_density_comparison(sxx1: &Array1<f64>, sxx2: &Array1<f64>) -> Result<f64, TimeSeriesProcessingErrorKind> {
    if sxx1.len() != sxx2.len() {
        return Err(TimeSeriesProcessingErrorKind::SeriesAreNotSameLength);
    }
